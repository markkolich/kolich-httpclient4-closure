...
For most applications, the synchronous `HttpClient` closure is acceptable.  However, if you are building a highly scalable, asynchronous, non-blocking application or API, then the asynchronous `HttpAsyncClient` closure is likely a better choice. 
